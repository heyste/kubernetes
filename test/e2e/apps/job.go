	...
	*/
	ginkgo.It("should manage the lifecycle of a job", func() {
		...
		c := watchEventConfig{
			...
			extJob:          patchedJob,
			internalKind:    "label",
			internalValue:   jobName,
			externalValue:   "patched",
		}
		waitForJobEvent(c)
		...
		c = watchEventConfig{
			...
			extJob:          &testJob,
			internalKind:    "label",
			internalValue:   "e2e-job-label",
			externalValue:   jobName,
		}
		waitForJobEvent(c)
...
func waitForJobEvent(config watchEventConfig) {
	...
	_, err := watchtools.Until(ctx, config.resourceVersion, config.w, func(event watch.Event) (bool, error) {
		if job, ok := event.Object.(*batchv1.Job); ok {
			...
			switch config.internalKind {
			case "annotation":
				internalValue = job.Annotations[config.internalValue]
			case "label":
				internalValue = job.Labels[config.internalValue]
			}

			found := job.ObjectMeta.Name == config.extJob.ObjectMeta.Name &&
				job.ObjectMeta.Namespace == f.Namespace.Name &&
				internalValue == config.externalValue &&
