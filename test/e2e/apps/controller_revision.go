...
var _ = SIGDescribe("Controller revision [Serial]", func() {
	...
	ginkgo.It("explore kubectl create daemonset", func() {
		test := "test/fixtures/doc-yaml/user-guide"
		dsYaml := readFile(test, "daemonset.yaml")

		framework.RunKubectlOrDieInput(ns, dsYaml, "create", "-f", "-")
